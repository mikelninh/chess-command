...
let qmate=C.make(C.fromFEN('7k/8/5K2/8/8/8/8/6Q1 w - - 0 1'),'g1g7');assert(qmate,'queen endgame drill move must be legal');assert(C.status(qmate).checkmate,'Qg7 must mate in curriculum position');
let rmate=C.make(C.fromFEN('7k/8/6K1/8/8/8/8/R7 w - - 0 1'),'a1a8');assert(rmate,'rook endgame drill move must be legal');assert(C.status(rmate).checkmate,'Ra8 must mate in curriculum position');
let promo=C.make(C.fromFEN('8/P7/8/8/8/8/5k2/7K w - - 0 1'),'a7a8q');assert(promo,'promotion drill must be legal');assert.equal(C.pieceAt(promo,'a8'),'wQ','promotion drill must create a queen');
console.log('Chess core tests passed: legal moves, perft 20/400/8902, en passant, castling, checkmate, SAN/PGN, and V5 endgame drills.');
