...
def puzzle_check(d,w,h):
 ...
 m=d.execute_script("""const $=s=>document.querySelector(s),r=e=>{let x=e.getBoundingClientRect();return [x.left,x.top,x.right,x.bottom,x.width,x.height]},b=r($('#v12PuzzleBoard')),c=r($('.v12-coach')),t=r($('.topbar')),main=r($('#puzzles .v12-main')),side=window.ChessPuzzleViewV12.side(),badge=$('#v12SideBadge'),z=[];if(Math.abs(b[4]-b[5])>2)z.push('board not square');if(b[4]<390)z.push('board too small');if(b[1]<t[3]-1||b[3]>innerHeight+1)z.push('board outside viewport');if(c[3]>innerHeight+1)z.push('coach clipped');if(main[3]>innerHeight+1)z.push('puzzle main clipped');if(document.documentElement.scrollWidth>innerWidth+1)z.push('horizontal scroll');if(getComputedStyle(document.body).overflowY!=='hidden')z.push('body can scroll');const white=[...document.querySelectorAll('.v12-actions button:not([hidden])')].filter(x=>{const q=getComputedStyle(x).backgroundColor;return q==='rgb(239, 239, 239)'||q==='rgb(255, 255, 255)'}).map(x=>x.textContent.trim());if(white.length)z.push('browser-white buttons '+white.join('/'));if(!badge?.textContent.includes(side==='w'?'WHITE':'BLACK'))z.push('side badge mismatch');if(!$('#v12Prompt')?.textContent.includes('You play '+(side==='w'?'White':'Black')))z.push('prompt side unclear');if(!$('#v12How')?.textContent.toLowerCase().includes('drag'))z.push('move instruction missing');if(!$('#v12Next')?.hidden)z.push('next visible before solve');return {z,b,c,t,main,side};""")
 ...
 print(f"Desktop PUZZLES PASS {w}x{h}: board={round(m['b'][4])}px side={m['side']}",flush=True)
...
def check(d,w,h):
 ...
 wait(d,"document.readyState!=='loading'&&window.ChessPuzzleViewV12&&window.ChessPieceSkins&&document.querySelector('.mobile-gamebar')&&document.body.classList.contains('viewport-play-active')")
