...
let ws=null;
const hard=setTimeout(()=>{console.error('Mobile visual gate exceeded 45 seconds.');try{ws?.close()}catch{};try{browser.kill('SIGKILL')}catch{};try{server.kill('SIGKILL')}catch{};process.exit(124)},45000);

async function pollJson(url,tries=80){let last;for(let i=0;i<tries;i++){try{const r=await fetch(url);if(r.ok)return await r.json();last=new Error('HTTP '+r.status)}catch(e){last=e}await sleep(75)}throw last||new Error('debug endpoint unavailable')}
async function pollHttp(url,tries=80){let last;for(let i=0;i<tries;i++){try{const r=await fetch(url,{cache:'no-store'});if(r.ok)return true;last=new Error('HTTP '+r.status)}catch(e){last=e}await sleep(75)}throw last||new Error('app server unavailable')}
...
try{
 ...
 async function waitEval(expression,label,tries=70){for(let i=0;i<tries;i++){if(await evalJs(expression))return true;await sleep(75)}const diag=await evalJs(`({href:location.href,title:document.title,ready:document.readyState,hasCore:!!window.ChessCore,hasCommand:!!window.ChessCommand,hasPuzzleCore:!!window.PuzzleCoreV12,scripts:[...document.scripts].map(s=>s.src).filter(Boolean),body:document.body.innerText.slice(0,500)})`);throw new Error('Timed out waiting for '+label+' '+JSON.stringify(diag)+' runtime='+JSON.stringify(runtimeErrors.slice(-5)))}
 async function run(width,height){
   ...
   console.log(`Mobile visual smoke passed at ${width}x${height}: board ${Math.round(metrics.board.width)}px; coach/nav clearance ${Math.round(metrics.nav.top-metrics.coach.bottom)}px; files ${metrics.files}.`);
 }
 await run(360,760);await run(390,844);
 console.log('Mobile visual gate passed.');
}finally{
 clearTimeout(hard);try{ws?.close()}catch{};try{browser.kill('SIGKILL')}catch{};try{server.kill('SIGKILL')}catch{};
}
