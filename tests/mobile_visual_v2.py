...
def check(d,w,h):
 ...
 print(f'VISUAL navigation returned {w}x{h}',flush=True)
 ...
 print(f"Mobile visual PASS {w}x{h}: board={round(m['b'][4])}px clearance={round(m['n'][1]-m['c'][3])}px orientation={m['f']}/{m['k']}",flush=True)
...
try:
 ...
 print('VISUAL local server ready',flush=True)
 ...
 print('VISUAL starting Chrome',flush=True)
 d=webdriver.Chrome(options=o);d.set_page_load_timeout(10);d.set_script_timeout(8);print('VISUAL Chrome ready',flush=True)
