...
// Undo should not create recursively exploding history.
s = T.undo(s);
assert(s.turn === 'w', 'undo restores previous turn');
assert(s.history.length === 0, 'undo restores compact history');

// Kings are not capturable: game completion is via checkmate.
s = T.start();
...
// FRS king-side castling is the King/Rook swap.
s = T.start();
...
// Attack-board rules: empty or a single owned pawn, player must retain a pawn.
s = T.start();
clearAttackBoardExcept(s, 'WQ', 2); // keep white rook pawn at a1(3)
...
// Pawn riding an attack board must count as moving and can promote.
...
// Standard en-passant works with the actual path the pawn traversed.
s = T.start();
